...
def _get_quantization_keys(model: nn.Module) -> Dict[str, int]:
    ...
    for name, module in model.named_modules():
        ...
        if module_name == 'bitorch' or any((x in module.__class__.__name__ for x in ("PactAct", "QConv", "QLinaer", "QEmbeddingBag", "BEmbeddingBag", "BEmbedding"))):
            variables = vars(module)
            bitwidth = 1
            try:
              variables = vars(module.weight_quantization)
            except:
              try:
                variables = vars(module.embedding_weight_quantization)
              except:
                pass
